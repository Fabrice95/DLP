...
import antlr4.ILPMLgrammar1tme2Parser.DefinefunctionContext;
...
public class ILPMLListener implements ILPMLgrammar1tme2Listener{
	...
	@Override public void enterDefinefunction(DefinefunctionContext ctx) {}
	@Override public void enterAffectation(AffectationContext ctx) {}


	@Override
	public void exitAffectation(AffectationContext ctx) {
		ctx.node = factory.newAffectation(ctx.var.getText(), ctx.val.node);
	}
	...
	@Override
	public void exitDefinefunction(DefinefunctionContext ctx) {
		IASTexpression[] params = new IASTexpression[ctx.pars.size()];
		IASTexpression[] instructions = new IASTexpression[ctx.body.size()];
		
		for(int i=0; i< ctx.pars.size(); i++)
			params[i]=ctx.pars.get(i).node;
		
		for(int i=0; i< ctx.body.size(); i++)
			params[i]=ctx.body.get(i).node;
		
		ctx.node = factory.newFunction(ctx.nom.getText(), params, instructions);
	}

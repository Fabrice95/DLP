...
#define ILP_AllocateVector(length) \
  ILP_malloc(sizeof(ILP_Object)*length, &ILP_object_Vector_class)
...
/* ajout TME3 - vector */
#define ILP_makeVector(length, value) \
  ILP_make_vector(length, value)

#define ILP_vectorLength(vector) \
  ILP_vector_length(vector)

#define ILP_vectorGet(vector, index) \
  ILP_vector_get(vector, index)
...
extern ILP_Object ILP_make_vector (ILP_Object length, ILP_Object type);

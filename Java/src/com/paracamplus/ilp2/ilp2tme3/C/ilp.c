...
ILP_Object
ILP_make_vector (ILP_Object length, ILP_Object value)
{
  if ( ILP_isInteger(o) ) {
    ILP_Object result = ILP_AllocateVector(length);
     result->_content.asVector._length = length;
     int i;
     for(i=0; i<length; i++)
      memmove(result->_content.asVector.asILP_Object, s, 1);
     return result;
  } else {
    return ILP_domain_error("Invalid argument type for size", o);
  }
}
...
ILP_sinus (ILP_Object o)
{
  if ( ILP_isFloat(o) ){
    ILP_Object result = ILP_AllocateFloat();
    result->_content.asFloat = sin(o->_content.asFloat);
    return result;
  } else if ( ILP_isInteger(o) ) {
    ILP_Object result = ILP_AllocateFloat();
    result->_content.asFloat = sin((double)o->_content.asInteger);
    return result;
  } else {
    return ILP_domain_error("Not a number", o);
  }
...
ILP_Object ILP_vector_length (ILP_Object vector);
ILP_Object ILP_vector_get (ILP_Object vector, ILP_Object index);
